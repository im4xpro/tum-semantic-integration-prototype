...
def load_gold_mapping(gold_mapping_path: Path) -> MappingDocument:
    return MappingDocument.model_validate(json.loads(gold_mapping_path.read_text()))
...
def build_gold_extraction_results(
    ...
    # Gold standard is derived via the same EntityExtractor every real run uses —
    # only the mapping itself differs, not the extraction pipeline.
    mapping = load_gold_mapping(gold_mapping_path)
